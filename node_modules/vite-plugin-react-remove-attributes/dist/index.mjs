...
// src/index.ts
function VitePluginReactRemoveAttributes({
  include = [/\.[cm]?[tj]sx?$/],
        ...
        if (filterValidFile(id)) {
          ...
          walk(ast, {
            enter(node, _parent, _prop, _index) {
              if (node.type === "Property" && "key" in node && node.key.type == "Literal" && attributes.includes(node.key.value)) {
                this.remove();
              }
            }
          });
          const map = new import_source_map.SourceMapGenerator({ file: id });
